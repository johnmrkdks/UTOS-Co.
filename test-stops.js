...
const dbPath = path.join(
	__dirname,
	"apps/server/.wrangler/state/v3/d1/miniflare-D1DatabaseObject/8682a4c3-fcc3-4435-9a31-1b710dd1a673.sqlite",
);
...
try {
	const tableInfo = db
		.prepare(
			"SELECT name FROM sqlite_master WHERE type='table' AND name='booking_stops'",
		)
		.get();
  ...
  `)
	...
	if (stopsDetails.length > 0) {
		...
		stopsDetails.forEach((stop) => {
			console.log(
				`  • Booking ${stop.bookingId}, Stop ${stop.stopOrder}: ${stop.address}`,
			);
		});
	} else {
