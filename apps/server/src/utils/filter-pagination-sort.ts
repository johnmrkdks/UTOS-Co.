...
import { z } from "zod";
import qs from "qs";
import { generatePaginationMetadata } from "./pagination-metadata";
...
export async function advancedQuery<
	...
	U extends
		| T["_"]["columns"]
		| Omit<T["_"]["columns"], keyof T["_"]["columns"]>,
>(
	db: DB,
	table: T,
	...
	const totalCount = (count[0]?.totalCount as number) || 0;

	const metadata = generatePaginationMetadata({
		page: fps.page,
		pageSize: fps.pageSize,
		totalCount,
	});
