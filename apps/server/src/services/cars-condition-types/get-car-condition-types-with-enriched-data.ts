...
export async function getCarConditionTypesWithEnrichedDataService(db: DB, params: ResourceList): Promise<QueryListResult<CarConditionTypeWithEnrichedData>> {
	...
	const data = carConditionTypes.data.map(conditionType => {
		return {
			...conditionType,
			metadata: {
				carsCount: carsCount.find(car => car.conditionTypeId === conditionType.id)?.count || 0,
			}
