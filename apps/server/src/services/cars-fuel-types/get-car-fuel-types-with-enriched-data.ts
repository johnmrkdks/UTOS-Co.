...
export async function getCarFuelTypesWithEnrichedDataService(db: DB, params: ResourceList): Promise<QueryListResult<CarFuelTypeWithEnrichedData>> {
	...
	const data = carFuelTypes.data.map(fuelType => {
		return {
			...fuelType,
			metadata: {
				carsCount: carsCount.find(car => car.fuelTypeId === fuelType.id)?.count || 0,
			}
