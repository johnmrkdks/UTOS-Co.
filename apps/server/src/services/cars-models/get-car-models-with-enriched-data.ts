...
export async function getCarModelsWithEnrichedDataService(
	db: DB,
	params: ResourceList,
): Promise<QueryListResult<CarModelWithEnrichedData>> {
	...
	const data = carModelsWithBrand.data.map((model) => {
		return {
			...
			metadata: {
				carsCount:
					carsCount.find((car) => car.modelId === model.id)?.count || 0,
			},
		};
	});
