...
export const DeleteDriverServiceSchema = z
	.object({
...
export type DeleteDriverServiceInput = z.infer<
	typeof DeleteDriverServiceSchema
>;
...
export async function deleteDriverService(
	...
	const { id, userId, forceDelete, confirmationText } =
		DeleteDriverServiceSchema.parse(input);
	...
	if (driverToDelete && targetUserId) {
		...
		if (
			(upcomingBookings.length > 0 || inProgressBookings.length > 0) &&
			!forceDelete
		) {
			throw new Error(
				JSON.stringify({
					...
					message:
						"Driver has active or upcoming bookings and cannot be deleted",
					details: {
	...
	if (targetUserId) {
		try {
			...
			await db
				...
				.where(eq(schema.accounts.userId, targetUserId));
			...
			await db
				...
				.where(eq(schema.bookings.userId, targetUserId));
			...
			console.log(
				`Successfully removed user ${targetUserId} and all related records`,
			);
		} catch (authError) {
			console.error("Failed to remove user:", authError);
			console.error("Error details:", authError);
			throw new Error(
				`Failed to remove user account: ${authError instanceof Error ? authError.message : "Unknown error"}`,
			);
		}
	}
