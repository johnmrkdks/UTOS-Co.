...
export async function getCarBodyTypesWithEnrichedDataService(
	db: DB,
	params: ResourceList,
): Promise<QueryListResult<CarBodyTypeWithEnrichedData>> {
	...
	const data = carBodyTypes.data.map((bodyType) => {
		return {
			...
			metadata: {
				carsCount:
					carsCount.find((car) => car.bodyTypeId === bodyType.id)?.count || 0,
			},
		};
	});
