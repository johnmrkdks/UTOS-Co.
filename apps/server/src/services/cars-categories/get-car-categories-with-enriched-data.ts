...
export async function getCarCategoriesWithEnrichedDataService(
	db: DB,
	params: ResourceList,
): Promise<QueryListResult<CarCategoryWithEnrichedData>> {
	...
	const data = carCategories.data.map((category) => {
		return {
			...
			metadata: {
				carsCount:
					carsCount.find((car) => car.categoryId === category.id)?.count || 0,
			},
		};
	});
