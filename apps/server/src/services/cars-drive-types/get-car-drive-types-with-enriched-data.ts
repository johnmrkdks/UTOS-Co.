...
export async function getCarDriveTypesWithEnrichedDataService(db: DB, params: ResourceList): Promise<QueryListResult<CarDriveTypeWithEnrichedData>> {
	...
	const data = carDriveTypes.data.map(driveType => {
		return {
			...driveType,
			metadata: {
				carsCount: carsCount.find(car => car.driveTypeId === driveType.id)?.count || 0,
			}
