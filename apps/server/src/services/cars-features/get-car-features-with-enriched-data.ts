...
export async function getCarFeaturesWithEnrichedDataService(
	db: DB,
	params: ResourceList,
): Promise<QueryListResult<CarFeatureWithEnrichedData>> {
	...
	const data = carFeatures.data.map((feature) => {
		return {
			...
			metadata: {
				carsCount:
					carsCount.find((car) => car.featureId === feature.id)?.count || 0,
			},
		};
	});
