...
export async function getCarBrandsWithEnrichedDataService(
	db: DB,
	params: ResourceList,
): Promise<QueryListResult<CarBrandWithEnrichedData>> {
	...
	const data = carBrands.data.map((brand) => {
		return {
			...
			metadata: {
				modelCount:
					carModelsCount.find((model) => model.brandId === brand.id)?.count ||
					0,
				carsCount:
					carsCount.find((car) => car.brandId === brand.id)?.count || 0,
			},
		};
	});
