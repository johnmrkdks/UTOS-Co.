...
import {
	type SecureQuoteData,
	storeSecureQuote,
} from "@/services/quotes/instant-quote-storage";
...
export type CalculateInstantQuoteParams = z.infer<
	typeof CalculateInstantQuoteSchema
>;
...
export async function calculateInstantQuoteService(
	...
	console.log(
		"🔍 calculateInstantQuoteService - Input data:",
		JSON.stringify(data, null, 2),
	);
	console.log(
		"🔍 calculateInstantQuoteService - Environment:",
	...
	try {
		console.log(
			"🔍 calculateInstantQuoteService - Attempting Google Maps API call",
		);
		...
		console.log(
			"🔍 calculateInstantQuoteService - Destinations:",
			destinations,
		);
		...
		console.log(
			"🔍 calculateInstantQuoteService - Total distance:",
			totalDistance,
			"meters",
		);
		console.log(
			"🔍 calculateInstantQuoteService - Total duration:",
			totalDuration,
			"seconds",
		);
	} catch (error) {
		console.error(
			"❌ calculateInstantQuoteService - Google Maps API failed:",
			error,
		);
	...
	console.log(
		"🔍 calculateInstantQuoteService - Fetching pricing configuration",
	);
	...
	if (data.carId) {
		console.log(
			"🔍 calculateInstantQuoteService - Looking for car-specific pricing",
		);
		...
		console.log(
			"🔍 calculateInstantQuoteService - Car-specific pricing results:",
			pricingConfigResult.length,
		);
		...
		if (pricingConfigResult.length === 0) {
			...
			pricingConfigResult = await db.select().from(pricingConfig).limit(1);
		}
	} else {
		console.log(
			"🔍 calculateInstantQuoteService - No car ID provided, using first available pricing configuration",
		);
	...
	console.log(
		"🔍 calculateInstantQuoteService - Final pricing config results:",
		pricingConfigResult.length,
	);

	if (pricingConfigResult.length === 0) {
		console.error(
			"❌ calculateInstantQuoteService - No pricing configuration found",
		);
		throw new Error(
	...
	if (distanceKm <= pricing.firstKmLimit) {
		...
		firstKmFare = Number.parseFloat(pricing.firstKmRate.toFixed(2));
		...
		additionalKmFare = Number.parseFloat(
			(additionalDistance * pricing.additionalKmRate).toFixed(2),
		);
	...
	console.log("🔍 calculateInstantQuoteService - Final calculation results:", {
		...
		additionalKmFare,
		totalAmount,
		estimatedDistance: Number.parseFloat((totalDistance / 1000).toFixed(3)),
