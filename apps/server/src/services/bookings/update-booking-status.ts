...
export async function updateBookingStatusService(
	db: DB,
	data: UpdateBookingStatusParams,
	env?: Env,
) {
	console.log(
		`🔍 UPDATE STATUS DEBUG: Starting updateBookingStatusService for booking ${data.id} with status ${data.status}`,
	);
	...
	try {
		...
		if (statusesRequiringCar.includes(data.status)) {
			console.log(
				`🚗 UPDATE STATUS DEBUG: Status requires car validation: ${data.status}`,
			);
			...
			console.log(
				`📋 UPDATE STATUS DEBUG: Found booking ${data.id}, carId: ${existingBooking.carId}`,
			);

			if (!existingBooking.carId) {
				console.error(
					`❌ UPDATE STATUS DEBUG: No car assigned to booking ${data.id}`,
				);
			...
			console.log(
				`✅ UPDATE STATUS DEBUG: Car validation passed for booking ${data.id}`,
			);
		}
		...
		switch (data.status) {
			...
			case BookingStatusEnum.DriverAssigned:
				if (!data.driverId) {
					throw ErrorFactory.badRequest(
						"Driver ID is required when assigning a driver",
					);
				}
		...
		console.log(
			"💾 UPDATE STATUS DEBUG: Updating booking in database with data:",
			JSON.stringify(updateData, null, 2),
		);
		const result = await updateBooking(db, { id: data.id, data: updateData });
		console.log(
			`✅ UPDATE STATUS DEBUG: Database update completed successfully for booking ${data.id}`,
		);

		// Send email notifications after successful booking update
		console.log(
			`📧 EMAIL DEBUG: Checking email notification conditions for booking ${data.id}`,
		);
		console.log(
			`📧 EMAIL DEBUG: env available: ${!!env}, result available: ${!!result}, status: ${data.status}`,
		);
		if (!env) {
			console.error(
				"❌ EMAIL DEBUG: env is missing - emails will not be sent. Check tRPC context passes env.",
			);
		}

		if (env && result) {
			console.log(
				`✅ EMAIL DEBUG: Entering email notification block for status ${data.status}`,
			);
			try {
				switch (data.status) {
					case BookingStatusEnum.Confirmed:
						...
						try {
							...
							console.log(
								`✅ Booking confirmation email sent for booking ${data.id}`,
							);
						} catch (emailError) {
							console.error(
					...
					case BookingStatusEnum.DriverAssigned:
						console.log(
							`🚗 EMAIL DEBUG: Driver assignment case - driverId: ${data.driverId}`,
						);
						if (data.driverId) {
							console.log(
								`📧 EMAIL DEBUG: Calling sendDriverAssignmentNotification for booking ${data.id}, driver ${data.driverId}`,
							);
							...
							console.log(
								`✅ Driver assignment notification sent for booking ${data.id}`,
							);
						} else {
							console.log(
								"❌ EMAIL DEBUG: No driverId provided for driver assignment",
							);
						}
						break;
					...
					case BookingStatusEnum.Cancelled:
						...
						console.log(
							`📧 EMAIL DEBUG: Sending cancellation email to client for booking ${data.id}`,
						);
						...
						console.log(
							`✅ Cancellation email sent to client for booking ${data.id}`,
						);
						break;
					...
					case BookingStatusEnum.Pending:
						...
						console.log(
							`⏭️ EMAIL DEBUG: Skipping client email for status: ${data.status}`,
						);
						break;
					...
					case BookingStatusEnum.Completed:
						...
						console.log(
							`📧 EMAIL DEBUG: Sending completion email to client for booking ${data.id}`,
						);
						await sendTripStatusNotification({
							bookingId: data.id,
							status: "completed",
						...
						console.log(
							`✅ Completion email sent to client for booking ${data.id}`,
						);
						break;

					default:
						console.log(
							`⏭️  EMAIL DEBUG: No email notification case for status: ${data.status}`,
						);
						break;
				}
			} catch (emailError) {
				...
				console.error(
					`❌ EMAIL DEBUG: Failed to send email notification for booking ${data.id}:`,
					emailError,
				);
				console.error(
					"❌ EMAIL DEBUG: Email error stack:",
					emailError instanceof Error ? emailError.stack : "No stack trace",
				);
			}
		} else {
			console.log(
				`⏭️  EMAIL DEBUG: Skipping email notifications - env: ${!!env}, result: ${!!result}`,
			);
		}

		console.log(
			`✅ UPDATE STATUS DEBUG: Successfully completed updateBookingStatusService for booking ${data.id}`,
		);
		return result;
	} catch (error) {
		console.error(
			`❌ UPDATE STATUS DEBUG: Error in updateBookingStatusService for booking ${data.id}:`,
			error,
		);
		console.error(
			"❌ UPDATE STATUS DEBUG: Error stack:",
...
export async function assignDriverService(
	...
	return await updateBookingStatusService(
		db,
		{
			id: data.bookingId,
