...
export async function getCarTransmissionTypesWithEnrichedDataService(db: DB, params: ResourceList): Promise<QueryListResult<CarTransmissionTypeWithEnrichedData>> {
	...
	const data = carTransmissionTypes.data.map(transmissionType => {
		return {
			...transmissionType,
			metadata: {
				carsCount: carsCount.find(car => car.transmissionTypeId === transmissionType.id)?.count || 0,
			}
