...
export function BookingTypeBadge({
	...
	const bookingType =
		booking.bookingType || (booking.packageId ? "package" : "custom");
		...
		switch (bookingType) {
			case "package": {
				...
				const clientBookedHours =
					isHourly && booking.estimatedDuration
						? Math.round(booking.estimatedDuration / 60)
						: null;

				let text = "Service";
				if (isHourly) {
					text = clientBookedHours
						? `Hourly (${clientBookedHours}h)`
						: "Hourly";
	...
	return (
		<Badge
			className={`px-2 py-0.5 text-xs ${badgeConfig.className} ${className || ""}`}
		>
			{badgeConfig.text}
		</Badge>
	);
