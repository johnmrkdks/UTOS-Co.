...
export function DateTimePicker({
		...
		<Popover>
			<PopoverTrigger asChild>
				<Button
					variant="outline"
					className={cn(
						"h-12 w-full justify-start text-left text-base",
	...
	const DesktopTimePicker = () => {
		...
		const canConfirm =
			selectedHour !== null &&
			selectedMinute !== null &&
			!isTimeInPast(selectedHour, selectedMinute, selectedPeriod);
					...
					<div className="space-y-3">
						...
						<div className="flex items-center justify-between">
							...
							<div className="flex gap-1">
								<button
									...
									className={cn(
										"rounded border px-2 py-0.5 font-medium text-xs transition-colors",
										selectedPeriod === "AM"
						...
						<div className="space-y-1.5">
							<p className="text-muted-foreground text-xs">
								Enter hour (1-12) and minute (0-59)
							</p>
							<div className="flex items-center gap-2">
								...
								<Input
									...
									onChange={(e) => {
										...
										if (
											val === "" ||
											(Number.parseInt(val) >= 0 && Number.parseInt(val) <= 59)
										) {
											setManualMinute(val);
										}
									}}
								...
								{hours.map((hour) => {
									...
									return (
										<button
											...
											disabled={isPast}
											className={cn(
												"h-7 rounded border font-medium text-xs transition-colors",
												selectedHour === hour
			...
			>
				...
				<DrawerContent onOpenAutoFocus={(e) => e.preventDefault()}>
					...
					<div className="space-y-2 px-4 pb-3">
						...
						<div className="relative">
							{!tempTime && (
								<div className="pointer-events-none absolute inset-0 z-10 flex items-center bg-background px-3 text-base text-muted-foreground">
									Tap to select time
								</div>
							)}
							<Input
								...
								className={cn(
									"h-12 appearance-none bg-background text-base [&::-webkit-calendar-picker-indicator]:hidden [&::-webkit-calendar-picker-indicator]:appearance-none",
									!tempTime &&
						...
						<div className="space-y-1 pt-2 pb-4">
							{generateTimeOptions().map((time) => (
								<Button
									...
									className="h-11 w-full justify-start text-left text-base"
					...
					{isPastTime && (
						<div className="mx-4 mb-4 rounded-md border border-amber-200 bg-amber-50 p-3">
	...
	return (
		<div className={cn("grid grid-cols-1 gap-5 sm:grid-cols-2", className)}>
			...
			<div>
				...
				{dateError && (
					<p className="mt-1 font-medium text-red-500 text-sm">{dateError}</p>
				)}
			</div>
			...
			<div>
				...
				{timeError && (
					<p className="mt-1 font-medium text-red-500 text-sm">{timeError}</p>
				)}
			</div>
		</div>
	);
