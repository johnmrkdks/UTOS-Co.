...
export function SquarePaymentForm({
	...
	const cardRef = useRef<Awaited<ReturnType<PaymentsInstance["card"]>> | null>(
		null,
	);
	const applePayRef = useRef<Awaited<
		ReturnType<PaymentsInstance["applePay"]>
	> | null>(null);
			...
			if (tokenResult.status === "OK" && tokenResult.token) {
				...
				const errMsg =
					tokenResult.errors && Array.isArray(tokenResult.errors)
						? ((tokenResult.errors[0] as { message?: string })?.message ??
							"Payment failed")
		...
		<div className="space-y-4">
					...
					<>
						...
						<div className="w-full min-w-[140px]">
							<button
								...
								className="flex h-12 w-full min-w-[200px] cursor-pointer items-center justify-center gap-2 rounded-lg border-0 bg-black px-4 py-3 font-semibold text-base text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-50"
			...
			<form onSubmit={handleCardSubmit} className="space-y-4">
				...
				<p className="text-muted-foreground text-xs">
					Your payment method will be authorized now and charged after your trip
					is completed.
				</p>
				{paymentError && (
					<div
						role="alert"
						className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-red-800 text-sm dark:border-red-800 dark:bg-red-950/50 dark:text-red-200"
					>
