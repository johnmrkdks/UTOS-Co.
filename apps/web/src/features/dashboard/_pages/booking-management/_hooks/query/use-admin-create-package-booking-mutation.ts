...
export const useAdminCreatePackageBookingMutation = (
	onSuccessCallback?: () => void,
) => {
	const queryClient = useQueryClient();

	return useMutation(
		trpc.bookings.adminCreatePackageBooking.mutationOptions({
			onSuccess: (data) => {
				...
				if (paymentLinkSent) {
					toast.success(
						"Package booking created. Payment link sent to client.",
					);
				} else if (paymentLinkMessage) {
					toast.warning(
						`Package booking created, but payment link was not sent: ${paymentLinkMessage}`,
					);
				} else {
					toast.success(
						`Package booking created successfully for ${data?.customerName}`,
					);
				}
				onSuccessCallback?.();
			},
