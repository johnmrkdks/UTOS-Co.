...
const editBookingSchema = z.object({
	scheduledPickupDate: z
		.string({ message: "Please select a pickup date" })
		.min(1, "Please select a pickup date"),
	scheduledPickupTime: z
		.string({ message: "Please select a pickup time" })
...
export function EditBookingDialog({
	...
	React.useEffect(() => {
		if (booking) {
			...
			const sortedStops = (booking.stops || [])
				.sort(
					(a: { stopOrder: number }, b: { stopOrder: number }) =>
	...
	const addStop = () => {
		...
		setFormData((prev) => ({
			...
			stops: [
				...currentStops,
				{ address: "", stopOrder: currentStops.length + 1 },
			],
		}));
	};
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent
				className="!max-w-3xl flex max-h-[90vh] flex-col overflow-hidden p-0"
						...
						<div className="space-y-4">
							...
							<div className="space-y-3">
								...
								<p className="text-gray-500 text-xs">
									Add intermediate stops between pickup and destination (e.g.
									client requests an extra stop before the trip)
								</p>
								{(formData.stops || []).map((stop, index) => (
									<div key={index} className="flex items-center gap-2">
										<span className="w-8 shrink-0 font-medium text-gray-500 text-sm">
											Stop {index + 1}
										</span>
										<div className="min-w-0 flex-1">
											<GooglePlacesInput
												...
												onPlaceSelect={(place) => {
													...
													updateStopAddress(index, place.description, lat, lng);
												}}
								...
								<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
									<div>
										...
										<Input
											...
											onChange={(e) =>
												updateFormData("offloaderName", e.target.value)
											}
										/>
									</div>
									<div>
						...
						<div className="space-y-4">
							<div>
								...
								<p className="mt-1 text-gray-500 text-xs">
									Only visible to drivers and admins
								</p>
							</div>
							<div>
								<label className="mb-2 block font-medium text-gray-700 text-sm">
									Client Special Requests (Optional)
								</label>
								<Textarea
								...
								/>
								<p className="mt-1 text-gray-500 text-xs">
									Visible to customer and shown in booking confirmation
								</p>
							</div>
						</div>
					</div>
