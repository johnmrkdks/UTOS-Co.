...
export function CreateCustomBookingDialog() {
	const { isCreateCustomBookingDialogOpen, closeCreateCustomBookingDialog } =
		useBookingManagementModalProvider();
	const [quote, setQuote] = useState<QuoteResult | null>(null);
	const [isCalculatingQuote, setIsCalculatingQuote] = useState(false);
	const [currentFormData, setCurrentFormData] = useState<
		Partial<EnhancedFormData>
	>({});
	...
	const handleCalculateQuote = (formData: EnhancedFormData) => {
		if (
			...
			toast.error(
				"Please select locations from the Google Places dropdown suggestions",
				{
					description:
						"Make sure to click on a location from the dropdown list that appears when typing addresses",
				},
			);
			return;
		}
		...
		calculateQuoteMutation.mutate(mutationPayload, {
			...
			onError: (error: any) => {
				...
				toast.error("Failed to calculate quote", {
					description:
						error.message || "Please check your addresses and try again",
				});
			},
		});
	};
	...
	const selectedCar =
		currentFormData.carId && cars?.data
	...
	return (
		<Dialog open={isCreateCustomBookingDialogOpen} onOpenChange={handleClose}>
			<DialogContent
				className="!max-w-6xl flex max-h-[90vh] w-full flex-col overflow-hidden"
			...
			>
				<DialogHeader className="flex-shrink-0">
					<div className="flex items-center justify-between">
						<div>
							...
							<DialogDescription>
								Create custom bookings with Google Places integration, multiple
								stops, and accurate pricing.
							</DialogDescription>
						</div>
						<Button
							variant="ghost"
				...
				<div className="flex-shrink-0 border-t bg-background/95 p-4 backdrop-blur supports-[backdrop-filter]:bg-background/60">
						...
						<Button
							...
							onClick={() => {
								...
								const form = document.querySelector(
									"form[data-enhanced-booking-form]",
								) as HTMLFormElement;
								if (form) {
									form.requestSubmit();
								}
