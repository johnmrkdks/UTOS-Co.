	...
	const handleDragEnd = async (result: DropResult) => {
		...
		try {
			await reorderMutation.mutateAsync({
				packageId,
				routeOrders: updatedRoutes.map((r, i) => ({
					id: r.id,
					stopOrder: i + 1,
				})),
			});
			...
			// Revert on error
			setRoutes(
				routesQuery.data?.sort((a, b) => a.stopOrder - b.stopOrder) ?? [],
			);
		}
	};
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				<DialogHeader>
					...
					<DialogDescription>
						Configure route stops and service points for this package. Customers
						will follow this route during their service.
					</DialogDescription>
				</DialogHeader>
							...
							<CardHeader>
								<div className="flex items-center justify-between">
									<div>
										...
										<CardDescription>
											Drag and drop to reorder stops. The order determines the
											service sequence.
										</CardDescription>
									</div>
