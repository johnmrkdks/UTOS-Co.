...
export function DeleteBodyTypeDialog({
	bodyType,
	className,
}: DeleteBodyTypeDialogProps) {
	const [isDialogOpen, setIsDialogOpen] = useState(false);
	const { data: checkUsage, isLoading: isCheckingUsage } =
		useCheckCarBodyTypeUsageQuery({ id: bodyType.id });
					...
					{isCheckingUsage ? (
						...
						<>
							...
							{!isInUse && (
								<Alert>
									<AlertDescription>
										This body type is not currently in use and can be safely
										deleted.
									</AlertDescription>
								</Alert>
							)}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex items-center gap-2">
										...
										<FormLabel>
											Are you sure you want to delete the body type{" "}
											<span className="font-bold">{bodyType.name}</span>?
										</FormLabel>
										<FormMessage />
									</FormItem>
								)}
