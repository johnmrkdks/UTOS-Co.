...
export function CarsList() {
	...
	const filteredCars = useMemo(() => {
		...
		if (filters.brand) {
			filtered = filtered.filter(
				(car: any) => car.model?.brand?.name === filters.brand,
			);
		}
		...
		if (filters.category) {
			filtered = filtered.filter(
				(car: any) => car.category?.name === filters.category,
			);
		}
		...
		if (filters.availability !== "all") {
			...
			filtered = filtered.filter((car: any) => car.isAvailable === isAvailable);
		}
		...
		if (filters.publicationStatus !== "all") {
			if (filters.publicationStatus === "published") {
				...
				filtered = filtered.filter((car: any) => !car.isPublished);
		...
		if (filters.minPrice > 0) {
			filtered = filtered.filter(
				(car: any) => car.pricePerDay >= filters.minPrice,
			);
		}

		if (filters.maxPrice < 10000) {
			filtered = filtered.filter(
				(car: any) => car.pricePerDay <= filters.maxPrice,
			);
		}
	...
	const publicationStats = useMemo(() => {
		if (!allCars?.data)
			return { total: 0, published: 0, unpublished: 0, publishedWithIssues: 0 };

		const total = allCars.data.length;
		const published = allCars.data.filter(
			(car: any) =>
				car.isPublished &&
				car.isActive &&
				car.isAvailable &&
		...
		const unpublished = allCars.data.filter(
			(car: any) => !car.isPublished,
		).length;
		const publishedWithIssues = allCars.data.filter(
			(car: any) =>
				car.isPublished &&
				(!car.isActive || !car.isAvailable || car.status !== "available"),
		).length;
	...
	return (
		<div className="relative flex flex-col gap-0">
			{/* Sticky Toolbar */}
			<PaddingLayout className="sticky top-[85px] z-9 flex items-center justify-between gap-4 border-boder border-b bg-background">
				<CarsFilters
					filters={filters}
			...
			{/* Scrollable Content */}
			<PaddingLayout className="h-full overflow-y-auto">
				<div>
					{viewMode === "grid" && (
						<CarsListGrid
							cars={cars?.data}
