...
import { useCreateCarBodyTypeMutation } from "@/features/dashboard/_pages/car-management/_hooks/query/car-body-type/use-create-car-body-type-mutation";
import { useIsCarBodyTypeExistMutation } from "@/features/dashboard/_pages/car-management/_hooks/query/car-body-type/use-is-car-body-type-exist-mutation";
...
export function AddBodyTypeDialog() {
	const [isDialogOpen, setIsDialogOpen] = useState(false);
	const mutation = useCreateCarBodyTypeMutation();
	const checkNameMutation = useIsCarBodyTypeExistMutation();
		...
		<Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
			...
			<DialogContent showCloseButton={false} className="flex flex-col gap-8">
				<DialogHeader>
					...
					<DialogDescription>
						Enter the name of the new body type.
					</DialogDescription>
				</DialogHeader>
				<Form {...(form as any)}>
					<form
					...
					>
						...
						<DialogFooter>
							...
							>
								{mutation.isPending ? "Adding..." : "Add Body Type"}
							</Button>
						</DialogFooter>
					</form>
				</Form>
