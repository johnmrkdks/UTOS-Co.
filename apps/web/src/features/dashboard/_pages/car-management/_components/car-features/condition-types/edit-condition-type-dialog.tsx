...
import { useUpdateCarConditionTypeMutation } from "@/features/dashboard/_pages/car-management/_hooks/query/car-condition-type/use-update-car-condition-type-mutation"
...
type EditConditionTypeDialogProps = {
	conditionType: CarConditionType
}

const FormSchema = z.object({
	name: z.string().min(1, "Condition type is required").max(50, "Condition type must be less than 50 characters"),
})

type FormValues = z.infer<typeof FormSchema>

export function EditConditionTypeDialog({ conditionType }: EditConditionTypeDialogProps) {
	const [isDialogOpen, setIsDialogOpen] = useState(false)
	const mutation = useUpdateCarConditionTypeMutation()
	...
	// Reset form when dialog opens or conditionType changes
	useEffect(() => {
		if (isDialogOpen) {
			form.reset({
				name: conditionType.name,
	...
	}, [conditionType, form, isDialogOpen])
	...
	const nameValidation = useEntityNameValidation({
		...
		originalValue: conditionType.name,
	...
	const hasChanges = () => {
		...
		return (
			currentValues.name !== conditionType.name
		)
	}
