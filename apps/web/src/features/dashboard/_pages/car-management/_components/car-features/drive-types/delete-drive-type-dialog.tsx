...
export function DeleteDriveTypeDialog({
	driveType,
	className,
}: DeleteDriveTypeDialogProps) {
	const [isDialogOpen, setIsDialogOpen] = useState(false);
	const { data: checkUsage, isLoading: isCheckingUsage } =
		useCheckCarDriveTypeUsageQuery({ id: driveType.id });
					...
					{isCheckingUsage ? (
						...
						<>
							...
							{!isInUse && (
								<Alert>
									<AlertDescription>
										This drive type is not currently in use and can be safely
										deleted.
									</AlertDescription>
								</Alert>
							)}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex items-center gap-2">
										...
										<FormLabel>
											Are you sure you want to delete the drive type{" "}
											<span className="font-bold">{driveType.name}</span>?
										</FormLabel>
										<FormMessage />
									</FormItem>
								)}
