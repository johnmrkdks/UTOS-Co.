...
export function SimplifiedDriverOnboardingForm({
	userId,
	onSuccess,
}: SimplifiedDriverOnboardingFormProps) {
	...
	const onSubmit = async (data: DriverApplicationFormData) => {
		try {
			...
			if (
				isNaN(licenseExpiryDate.getTime()) ||
				isNaN(dateOfBirthDate.getTime())
			) {
			...
			toast.success(
				"Driver application submitted successfully! Your application is now pending admin review.",
			);
			onSuccess?.();
		} catch (error) {
					...
					<div className="flex items-center justify-between">
						...
						>
							{onboardingStatus.status === "completed" ? (
								...
								<>
									<Clock className="mr-1 h-4 w-4" /> Step {currentStep} of{" "}
									{totalSteps}
								</>
							)}
						</Badge>
						...
						return (
							<div key={step} className="flex items-center">
								<div className="flex flex-col items-center">
									...
									<span
										className={`mt-2 text-xs ${status === "current" ? "font-medium text-blue-600" : "text-gray-500"}`}
									>
										{stepTitles[step - 1]}
									</span>
								</div>
								{step < totalSteps && (
									<div
										className={`mx-4 h-0.5 w-20 ${status === "completed" ? "bg-green-500" : "bg-gray-200"}`}
									/>
								)}
							</div>
						);
							...
							<CardContent className="space-y-6">
								<div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
									<div className="flex items-start">
										<AlertCircle className="mt-0.5 mr-2 h-5 w-5 text-yellow-600" />
						...
						<Card>
							...
							<CardContent className="space-y-6">
								<div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
									<div className="flex items-start">
										...
										<div className="text-blue-800 text-sm">
											...
											<p className="mt-1">
												This person will be contacted only in emergencies or if
												we can't reach you during important situations.
											</p>
										</div>
									</div>
								</div>
								...
								<div className="rounded-lg border border-green-200 bg-green-50 p-4">
									<div className="flex items-start">
										<CheckCircle className="mt-0.5 mr-2 h-5 w-5 text-green-600" />
										<div className="text-green-800 text-sm">
											<p className="font-medium">Almost Done!</p>
											<p className="mt-1">
												After submission, an admin will review your application.
					...
					<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
						...
						<div className="rounded-lg bg-yellow-50 p-4 text-center">
							...
							<p className="text-sm text-yellow-700">
								If approved, you'll receive confirmation and next steps
							</p>
						</div>
						...
						<div className="rounded-lg bg-green-50 p-4 text-center">
							...
							<p className="text-green-700 text-sm">
								Once activated, you can start accepting bookings
							</p>
						</div>
					</div>
				</CardContent>
