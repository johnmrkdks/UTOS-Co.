...
export const useRemoveUserMutation = () => {
	...
	return useMutation(
		trpc.drivers.delete.mutationOptions({
			onSuccess: (data) => {
				...
				queryClient.invalidateQueries({
					predicate: (query) => {
						return query.queryKey.some(
							(key) =>
								typeof key === "string" &&
								(key.includes("drivers") || key.includes("users")),
						);
					},
				});
				...
				try {
					...
					if (errorData.type === "DRIVER_HAS_ACTIVE_BOOKINGS") {
						...
						toast.error("Confirmation required", {
							description:
								"Use the comprehensive deletion dialog for proper confirmation and validation.",
						});
					} else {
