...
export const useVerifyDriverDocumentsMutation = () => {
	...
	return useMutation(trpc.drivers.verifyDocuments.mutationOptions({
		onSuccess: (data) => {
			...
			const statusMessage = 
				data?.verificationStatus === "verified" ? "Driver documents verified and approved" :
				data?.verificationStatus === "rejected" ? "Driver verification rejected" :
				data?.verificationStatus === "needs_revision" ? "Driver needs to revise documents" :
