...
interface UnifiedBookServicePageProps {
	serviceId?: string;
}
...
export function UnifiedBookServicePage({ serviceId: propServiceId }: UnifiedBookServicePageProps) {
	...
	const handleSubmit = async () => {
		...
		console.log("📦 Unified booking payload:", bookingPayload);

		try {
			const result = await createBookingMutation.mutateAsync(bookingPayload);
			console.log("✅ Unified booking successful:", result);
			setStep("confirmation");
		} catch (error) {
			console.error("❌ Unified booking failed:", error);
		}
	};
