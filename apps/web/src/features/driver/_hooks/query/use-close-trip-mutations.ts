...
export const useCloseTripWithExtrasMutation = () => {
	...
	return useMutation(
		trpc.bookings.closeTripWithExtras.mutationOptions({
			onSuccess: (data) => {
			...
			onError: (error) => {
				...
				toast.error("Failed to complete trip", {
					description:
						error.message || "An error occurred while completing the trip",
				});
			},
		}),
	);
...
export const useCloseTripWithoutExtrasMutation = () => {
	...
	return useMutation(
		trpc.bookings.closeTripWithoutExtras.mutationOptions({
			onSuccess: (data) => {
				// Invalidate and refetch related queries to ensure UI updates
				queryClient.invalidateQueries({
					queryKey: trpc.bookings.getDriverBookings.queryKey(),
