...
function ensureGoogleMapsLoaded(): Promise<void> {
	...
	if (!apiKey) {
		loadPromise = Promise.reject(
			new Error("VITE_GOOGLE_MAPS_API_KEY is not set"),
		);
		return loadPromise;
	}
	...
	const getPlacePredictions = async (input: string): Promise<PlaceResult[]> => {
		...
		try {
			...
			const { AutocompleteSuggestion, AutocompleteSessionToken } =
				(await importLibrary("places")) as {
					AutocompleteSuggestion: AutocompleteSuggestionType;
		...
		try {
			...
			return {
				...
				description:
					...
					predictions[predictionIndex]?.description ??
					"",
				geometry,
			};
		} catch (err) {
